...
\item Choisir un exemple d'entraînement. La procédure de choix doit assurer que tous les exemples seront choisis.
...
\noindent Ainsi pour le premier neurone, on pose $u_1=W_{1,1}x_1+W_{1,2}x_2+b_1$ et $u_2=W_{2,1}x_1+W_{2,2}x_2+b_2$. On effectue la somme logique et on obtient par exemple la séparation de l'espace de la figure 6 ~\ref{Dessin}.
...
\begin{enumerate}
\item Tirer une donnée $x$ au hasard dans la base $\mathcal D$.
\item Appliquer le réseau à $x$ afin d'avoir $y$. Tirer $y$ si la sortie du réseau est aléatoire.
\item $ F \leftarrow \frac{n}{n+1} F+\frac{1}{n+1}\frac{\partial }{\partial w} l(y)\frac{\partial }{\partial w'}l(y)$
\end{enumerate}
\end{minipage}}\\
\\
...
\section{Comparaison de la descente de gradient Euclidienne et de celle Riemanienne sur l'exemple du XOR aléatoire}
\subsection{Modèle}
...
\label{fig:xorgrad}%le label neurone etait deja pris
\end{figure}
